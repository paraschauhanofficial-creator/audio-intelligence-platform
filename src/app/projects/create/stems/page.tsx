...
export default function CreateStemsPage() {
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    // Quick fingerprint each file (parallel, fast)
  ...
  const setSection = (id: string, section: StemSection) => {
    setStems(prev => prev.map(s => s.id === id
      ? { ...s, overrideSection: section, overrideSlot: SLOT_OPTIONS[section][0] }
      : s
    ));
  };
  ...
  const effectiveSection = (s: StemEntry): StemSection =>
    s.overrideSection ?? s.identification.section;

  const effectiveSlot = (s: StemEntry): string =>
    s.overrideSlot ?? s.identification.slot;
...
const { data: project, error: projErr } = await supabase
  ...
  return;
      ...
      if (projErr || !project) { setSaveMsg("Failed to create project."); return; }

      // 2. Upload reference mix if provided (muted, reference only)
      if (hasRefMix && refMixFile) {
        ...
        const { error: refErr } = await supabase.storage
          .from("project-files")
          .upload(refPath, refMixFile, { contentType: refMixFile.type });
        if (!refErr) {
          await supabase.from("projects").update({
            reference_mix_path: refPath,
          }).eq("id", project.id);
        }
      }

      // 3. Re-run slot indexing on confirmed assignments so duplicates are numbered correctly
      const tracker = createSlotIndexTracker();
      // Sort by section then slot for consistent ordering
      ...
      // 4. Upload each stem file + insert project_stems row
      for (let i = 0; i < sorted.length; i++) {
        ...
        if (uploadErr) {
          console.error("Upload failed for", entry.file.name, uploadErr);
          continue;
        }
      ...
      // 5. Navigate to stems project page (analysis runs there)
      setTimeout(() => {
        router.push(`/projects/${project.id}/stems`);
      }, 800);
  ...
  if (phase === "upload") return (
    <div className="min-h-screen bg-[#0A0A0A] text-white flex flex-col">
      <div className="h-[72px] border-b border-[#1F2937] px-8 flex items-center justify-between">
        <h1 className="text-2xl font-bold">New Stems Project</h1>
        ...
        <div className="w-full max-w-2xl space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-2">
              ...
              <input
                type="text" placeholder="My Stems Session"
                value={projectName} onChange={e => setProjectName(e.target.value)}
                className="bg-[#111827] border border-[#1F2937] rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-[#14D8C4] transition"
              />
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-xs text-zinc-500 uppercase tracking-wide">Genre</label>
              <input
                type="text" placeholder="Hip-Hop, Pop, Rock..."
          ...
          <div className="bg-[#111827] border border-[#1F2937] rounded-xl p-4 flex items-center justify-between">
            ...
            <button onClick={() => setHasRefMix(v => !v)}
              className={`w-11 h-6 rounded-full border transition relative ${hasRefMix ? "border-[#14D8C4] bg-[#14D8C430]" : "border-[#1F2937] bg-[#0A0A0A]"}`}>
              <div className={`absolute top-0.5 w-5 h-5 rounded-full transition-all ${hasRefMix ? "left-5 bg-[#14D8C4]" : "left-0.5 bg-zinc-600"}`}/>
            </button>
          </div>

          {hasRefMix && (
            <div
              onClick={() => refMixInputRef.current?.click()}
              className="border border-dashed border-[#1F2937] rounded-xl p-4 flex items-center gap-3 cursor-pointer hover:border-[#14D8C440] transition">
              ...
              <input ref={refMixInputRef} type="file" accept="audio/*" className="hidden"
                onChange={e => setRefMixFile(e.target.files?.[0] ?? null)}/>
            </div>
          )}
          ...
          <div
            ...
            className="border-2 border-dashed border-[#1F2937] rounded-2xl p-16 flex flex-col items-center gap-4 cursor-pointer hover:border-[#14D8C440] transition group">
            ...
            <div className="px-6 py-2 rounded-lg text-sm font-semibold" style={{ backgroundColor: trackColor, color: "#000" }}>
              Browse Files
            </div>
  ...
  if (phase === "fingerprinting") return (
    <div className="min-h-screen bg-[#0A0A0A] text-white flex flex-col items-center justify-center gap-6">
      ...
      <div className="text-center">
        <p className="text-xl font-semibold" style={{ color: trackColor }}>Identifying Stems</p>
        <p className="text-sm text-zinc-400 mt-2">{fingerprintMsg}</p>
      </div>
      <div className="w-80 h-2 bg-[#1F2937] rounded-full overflow-hidden">
        <div className="h-full rounded-full transition-all" style={{ width: `${fingerprintPct}%`, backgroundColor: trackColor }}/>
      </div>
  ...
  if (phase === "saving") return (
    <div className="min-h-screen bg-[#0A0A0A] text-white flex flex-col items-center justify-center gap-6">
      ...
      <div className="text-center">
        <p className="text-xl font-semibold" style={{ color: masterColor }}>Saving Stems</p>
        <p className="text-sm text-zinc-400 mt-2">{saveMsg}</p>
      </div>
      <div className="w-80 h-2 bg-[#1F2937] rounded-full overflow-hidden">
        <div className="h-full rounded-full animate-pulse" style={{ width: "60%", backgroundColor: masterColor }}/>
      </div>
    </div>
  );

  // ─────────────────────────────────────────────────────────────────────────
  // RENDER — IDENTIFICATION POPUP PHASE
  // ─────────────────────────────────────────────────────────────────────────
    ...
    <div className="min-h-screen bg-[#0A0A0A] text-white flex flex-col">
      ...
      <div className="h-[72px] border-b border-[#1F2937] px-8 flex items-center justify-between flex-shrink-0">
        <div>
          ...
          <p className="text-xs text-zinc-500">{stems.length} stems detected · {needsReviewCount > 0 ? <span className="text-[#FF6B4A]">{needsReviewCount} need review</span> : <span className="text-[#14D8C4]">All identified</span>}</p>
        </div>
        <div className="flex items-center gap-3">
          {saveMsg && <span className="text-sm text-[#FF6B4A]">{saveMsg}</span>}
          <button onClick={() => { setPhase("upload"); setStems([]); }} className="px-4 py-2 rounded-lg border border-[#1F2937] text-sm">
            Start Over
          </button>
          <button
      ...
      <div className="flex-1 overflow-y-auto p-8">
        <div className="grid grid-cols-1 gap-3 max-w-5xl mx-auto">
              ...
              <div key={section}>
                ...
                <div className="space-y-2">
                  {sectionStems.map(entry => {
                    ...
                    const isUnidentified = entry.identification.method === "unidentified";

                    return (
                      <div key={entry.id}
                        className={`bg-[#111827] rounded-xl border p-4 flex items-center gap-4 transition ${
                          needsReview ? "border-[#FF6B4A60]" : "border-[#1F2937]"
                        }`}>

                        {/* Status dot */}
                        <div className={`w-2 h-2 rounded-full flex-shrink-0 ${needsReview ? "bg-[#FF6B4A]" : "bg-[#14D8C4]"}`}/>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            <span className="text-[10px] text-zinc-600">
                              via {entry.identification.method}
                            </span>
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col items-end gap-1 flex-shrink-0 w-20">
                          <span className="text-[10px] text-zinc-500">
                            {isUnidentified ? "Unknown" : `${Math.round(entry.identification.confidence * 100)}%`}
                          </span>
                          <div className="w-full h-1 bg-[#1F2937] rounded-full overflow-hidden">
                            <div className="h-full rounded-full transition-all" style={{
                              width: `${entry.identification.confidence * 100}%`,
                              backgroundColor: entry.identification.confidence > 0.75 ? "#14D8C4"
                                : entry.identification.confidence > 0.50 ? "#F0A500"
                                : "#FF6B4A",
                            }}/>
                          </div>
                        </div>
                        ...
                        <select
                          value={sec}
                          onChange={e => setSection(entry.id, e.target.value as StemSection)}
                          className="bg-[#0A0A0A] border border-[#1F2937] rounded-lg px-2 py-1.5 text-xs text-zinc-300 focus:outline-none focus:border-[#14D8C4] flex-shrink-0"
                        >
                          {(["drums","instruments","vocals","other"] as StemSection[]).map(s => (
                        ...
                        {/* Slot override */}
                        <select
                          value={slot.replace(/_\d+$/, "")}
                          onChange={e => setSlot(entry.id, e.target.value)}
                          className="bg-[#0A0A0A] border border-[#1F2937] rounded-lg px-2 py-1.5 text-xs text-zinc-300 focus:outline-none focus:border-[#14D8C4] flex-shrink-0"
                        >
                          {SLOT_OPTIONS[sec].map(s => (
                        ...
                        {entry.identification.runKeyDetection && (
                          <span className="text-[9px] border border-[#14D8C440] text-[#14D8C4] px-1.5 py-0.5 rounded flex-shrink-0">
                            Key ✓
                          </span>
                        )}
                        ...
                        <button onClick={() => removeEntry(entry.id)}
                          className="text-zinc-600 hover:text-[#FF6B4A] transition text-sm flex-shrink-0">✕</button>
                      </div>
                    );
                  })}
                </div>
          ...
          <div
            onClick={() => fileInputRef.current?.click()}
            className="mt-6 border border-dashed border-[#1F2937] rounded-xl p-6 flex items-center justify-center gap-3 cursor-pointer hover:border-[#14D8C440] transition text-zinc-500 hover:text-zinc-300">
