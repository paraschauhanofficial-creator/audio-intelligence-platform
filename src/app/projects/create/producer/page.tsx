  ...
  return (
    <div className="min-h-screen bg-[#0A0A0A] text-white">
      ...
      <div className="max-w-3xl mx-auto px-8 py-12">
        <h2 className="text-4xl font-bold mb-3">Create Producer Project</h2>
        <p className="text-zinc-400 mb-10">Start with an AI-generated mix and take full control of every aspect of the production process.</p>
        ...
        <div className="space-y-6">
                      ...
                      {files.map((file, index) => (
                        <div key={index} className="flex items-center justify-between bg-[#111827] border border-[#1F2937] rounded-lg px-3 py-2">
                          ...
                          <Trash2 size={16} onClick={() => setFiles(files.filter((_, i) => i !== index))}
                            className="text-zinc-400 hover:text-[#14D8C4] cursor-pointer transition"/>
                        </div>
                      ))}
                    </div>
                  </div>
                )}
                {uploading && <div className="mt-2 text-[#14D8C4]">Uploading files...</div>}
          ...
          <div className="flex gap-4">
            ...
            <button onClick={handleCreateProject} className="px-6 py-3 bg-[#14D8C4] text-black font-semibold rounded-xl">Create Project</button>
          </div>
        </div>
      </div>
    </div>
